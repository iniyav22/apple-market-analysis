    ...
    def run(self, user_prompt: str, max_retries: int = 2):
        ...
        if not eval_result.feasible:
            ...
            return {
                "status": "error",
                "feasible": False,
                "feasibility_reason": eval_result.reason,
...
DEFAULT_PROMPT = "Show Apple quarterly returns over time"

with st.form("vis_agent_form"):
...
if submitted and user_prompt:
    ...
    with st.chat_message("assistant"):
        with st.spinner("Generating visualization..."):
            ...
            st.write("Feasible:", result.get("feasible", False))
            st.write("Reason:", result.get("feasibility_reason", "No reason provided."))

            st.subheader("Code Validation")
            st.write("Generated code executed successfully:", result.get("code_valid", False))
